...
const CommentReactions = ({
  ...
  const [userHasClapped, setUserHasClapped] = useState(
    !!session.data?.user.userId && !!comment.claps.length,
  );
            ...
            <Alert
              message={
                <div className="text-sm">
                  <p>Please login to {action} this comment.</p>
                  <Link
                    href="/login"
  ...
  const handleReply = () => {
    if (handleGuestAction("reply")) return;

    setShowForm((prev) => !prev);
  };

  const handleShowReplies = () => {
    if (handleGuestAction("view replies")) return;
